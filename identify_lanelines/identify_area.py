...
def identify_lane_position(histogram, rolling_window=100, std=20.0, approx_left_lane=None, approx_right_lane=None, peak_filter_threshold=0.1, peak_filter_dot_threshold=0.1, peak_filter_dot_dot_threshold=0, lane_window=100):
    histogram_df = pd.DataFrame(histogram)
    histogram_df['Peak_Filter'] = histogram_df[0].rolling(window=rolling_window, axis=0, center=True, win_type='gaussian').mean(std=std)
    histogram_df['Peak_Filter_dot'] = histogram_df['Peak_Filter'].diff().abs()
    ...
    histogram_df['Lanes'] = np.where((histogram_df['Peak_Filter'] > peak_filter_threshold)
                                     & (histogram_df['Peak_Filter_dot'] <= peak_filter_dot_threshold)
    ...
    return histogram_df['Peak_Filter'], left_lane, right_lane
...
if __name__ == "__main__":
    ...
    for i in range(img_areas):
        ...
        lanes, left_lane, right_lane = identify_lane_position(histogram, std=20+((i/img_areas)*20), approx_left_lane=left_lane, approx_right_lane=right_lane, peak_filter_threshold=0, lane_window=100+((i/img_areas)*100))
        print(lanes)
        print(left_lane, right_lane)
